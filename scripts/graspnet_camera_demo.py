"""
Live RGB-D GraspNet demo for cameraws cameras.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="cameraws config YAML path",
    )
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument(
        "--yolo-model",
        default=None,
        help="override config yolo.model_name; relative names are loaded from cameraws/models",
    )
    parser.add_argument(
        "--yolo-device",
        default=None,
...
def _resolve_yolo_model_path(model_name: str) -> Path:
...
def load_yolo(cfg: dict, args: argparse.Namespace) -> tuple[Optional[Any], dict[str, Any]]:
    ...
    model_name = str(args.yolo_model or yolo_cfg.get("model_name", "yoloe-26s-seg.pt"))
    model_path = _resolve_yolo_model_path(model_name)
    device = args.yolo_device or yolo_cfg.get("device", "cpu")
